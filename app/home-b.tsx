...
export default function HomeB() {
  return (
    <div className="px-4 md:px-8 py-6 flex flex-col gap-4">
      ...
      {works.map((work, i) => (
        <AnimatedCard key={i} delay={i * 80}>
          <a
            ...
            style={{ background: "var(--tomato)", color: "var(--background)" }}
          >
            <div>
              <h2
              ...
              {work.subtitle && (
                <p
                  className="text-base md:text-lg font-semibold mt-1 opacity-80"
                  style={{ fontFamily: "var(--font-poppins)" }}
                >
                  {work.subtitle}
                </p>
            ...
            <div
              className="flex flex-wrap gap-x-6 gap-y-1 text-xs font-semibold uppercase tracking-widest opacity-70"
            ...
            <div className="flex flex-col md:flex-row md:items-end gap-5 md:gap-12">
              <p
                className="max-w-2xl text-sm leading-relaxed opacity-85"
                style={{ fontFamily: "var(--font-body)" }}
              >
                {work.description}
              </p>
      ...
      {/* Discrete Math */}
      <AnimatedCard delay={works.length * 80}>
        <div
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {canvaProjects.map((p) => (
              <a
                ...
                style={{ background: "rgba(44,81,76,0.10)" }}
              >
                <h3
